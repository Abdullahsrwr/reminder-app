...
class showChart extends StatefulWidget {
  const showChart({Key? key, required this.title, required this.data}) : super(key: key);
...
class _showChartState extends State<showChart>{
  ...
  Widget build(BuildContext context) {
    ...
    final taskDataList = [
      ...
      new taskData((getTasksPerDay(fireTaskList)[1]), weekdays[((DateTime.now().weekday+1)-1)%7]),
      new taskData((getTasksPerDay(fireTaskList)[2]), weekdays[((DateTime.now().weekday+2)-1)%7]),
      new taskData((getTasksPerDay(fireTaskList)[3]), weekdays[((DateTime.now().weekday+3)-1)%7]),
      new taskData((getTasksPerDay(fireTaskList)[4]), weekdays[((DateTime.now().weekday+4)-1)%7]),
      new taskData((getTasksPerDay(fireTaskList)[5]), weekdays[((DateTime.now().weekday+5)-1)%7]),
      new taskData((getTasksPerDay(fireTaskList)[6]), weekdays[((DateTime.now().weekday+6)-1)%7]),
    ];
    return Scaffold(
      appBar: AppBar(
